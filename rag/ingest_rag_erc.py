...
# Bootstrap d'import : ajoute la racine du repo pour importer src.config
project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

try:
    from src.config import require_mistral_api_key, VECTOR_DB_DIR
except ImportError:
    print(f"❌ ERREUR: Impossible d'importer src.config depuis {project_root}")
    sys.exit(1)
